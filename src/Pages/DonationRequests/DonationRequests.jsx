...
const DonationRequests = () => {
    ...
    if (loading) return <div className="min-h-screen flex justify-center items-center"><span className="loading loading-bars loading-lg text-red-600"></span></div>;
            ...
            <div className="max-w-7xl mx-auto">
                <div className="text-center mb-12">
                    <h2 className="text-3xl md:text-5xl font-extrabold text-gray-900 mb-4 tracking-tight">
                        Pending <span className="text-red-600 italic">Donation Requests</span>
                    </h2>
                    <p className="text-gray-600 max-w-2xl mx-auto">
                        Heroism is in your blood. Browse the requests below and help someone in their critical time.
                    </p>
                    <div className="h-1.5 w-20 bg-red-600 mx-auto mt-6 rounded-full"></div>
                </div>
                        ...
                        {requests.map((request) => (
                            <div key={request._id} className="bg-white rounded-3xl p-6 shadow-sm hover:shadow-xl transition-all border border-gray-100 group">
                                <div className="flex justify-between items-start mb-6">
                                    <div className="bg-red-50 text-red-600 font-bold px-4 py-1 rounded-full text-sm">
                                ...
                                <div className="space-y-3 mb-8">
                                    ...
                                    <div className="flex items-center gap-3 text-gray-600">
                                        <span className="text-lg">⏰</span>
                                        <span className="text-sm font-medium">{request.donation_time}</span>
                                    </div>
                                </div>

                                <Link 
                                    to={`/request-details/${request._id}`}
                                    className="block w-full text-center py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-red-600 transition-all active:scale-95 shadow-lg"
                                >
                                    View Details
                                </Link>
                            </div>
