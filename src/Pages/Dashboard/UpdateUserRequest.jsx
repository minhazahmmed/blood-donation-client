...
const UpdateUserRequest = () => {
  ...
  const [district, setDistrict] = useState(
    requestData?.recipient_district || ""
  );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await axiosSecure.patch(
        `/request/update/${requestData._id}`,
        updatedFormData
      );
      if (res.data.matchedCount > 0) {
        ...
        if (role === "admin") {
          navigate("/dashboard/all-donation-requests", { replace: true });
        } else {
          navigate("/dashboard/my-request", { replace: true });
        }
      }
    } catch (err) {
      console.error(err);
        ...
        <form className="p-5 sm:p-8 space-y-4" onSubmit={handleUpdate}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex flex-col">
              <label className="text-slate-700 text-sm font-semibold mb-1">
                Requester Name
              </label>
              <input
                type="text"
                defaultValue={requestData?.requester_name}
                readOnly
            ...
            <div className="flex flex-col">
              ...
              <input
                type="email"
                defaultValue={requestData?.requester_email}
                readOnly
                className="w-full p-2.5 rounded-lg border border-gray-200 bg-gray-50 text-slate-500 font-medium cursor-not-allowed"
              />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="flex flex-col">
              ...
              <select
                ...
                required
                className="w-full p-2.5 rounded-lg border border-gray-300 text-slate-700 font-medium"
              >
